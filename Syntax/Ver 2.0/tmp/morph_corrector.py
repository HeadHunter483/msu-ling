...
for j in range(len(word_root_mas)):
    if "PARTCP" in word_root_mas[j]:
            find_noun(int(index[j]),int(index[j+1]),"acc",0)
    if "A" in word_root_mas[j]:
            find_noun(int(index[j]),int(index[j+1]),"nom",1)
    if "V" in word_root_mas[j]:
            find_noun(int(index[j]),int(index[j+1]),"nom",2)
...
for i in range(len(conll)):            
    if conll[i][3]=="S":
        ...
        while(j<len(conll)):
            ...
            if conll[j][3]=="S":
                str4=find_padej(j)
                if str4!="gen" and str4!="ins" and str4!="acc":
