...
def check_templates(parts, templates):
    return any(check_template(parts, template) 
                    for template in templates)

def check_template(parts, template):
    return all(part in templ_part 
...
def mystem_postprocess(analyzed):
    """
    ...
    for analysis in analyzed:
        if '|' in analysis['text']: # the end of the line
            ...
            if current_sentence:
                ...
                sentences.append(post(current_sentence))
                current_sentence = []
                if pieces[1] != '':
