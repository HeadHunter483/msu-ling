...
int programm(char *path,string name){ // для  анализа,когда данные вводятся вручную.
	...
	for (int i = 0; i < n; i++){
		string line5 = "python ru-syntax-master.py " + name+" auto "+to_string(i+1)+" "+to_string(n);
...
void autoanalyzer(char *writable, string name){
	FILE *in, *out;
	char c, d;
	in = fopen("in.txt", "r");
	...
	programm(writable,name); // основной блок работы режима с ручным вводом текстов
...
void addmode(char *writable, string name) {
	ifstream file1;
	...
	if (text2 == "test") {
		...
		string path11 = "python ru-syntax-master.py " + name + " addmode " + to_string(1) + " " + to_string(1); // запускаем в режиме тест для 
																												//одного предложения								
		...
		string line431 = "cd " + name + "\\" + "bin" + "\\Malt\\" + "&& python converter.py test_"
...
void test_analyzer(char *writable, string name) {
	ifstream file1;
	ofstream fout;
	...
	for (int i = 0; i < kolvo_testov; i++) {
		string line5 = "python ru-syntax-master.py " + name + " test " + to_string(i + 1) + " " + to_string(kolvo_testov);
		const char *line_5 = line5.c_str();
...
int main(){
	...
	cout << "	------------------------------------------------------------------------" << endl;
	cout << "1.1.Укажите путь до директории ru-syntax-master:" << endl;
	getline(cin, name);
	if (name == ""){ //если задали пустой путь.Сообщить и выйти.
		cout << "	Путь до директории не задан.Работа программы завершена.";
		system("pause");
		exit(0);
	}
	if (name.find("ru-syntax-master") == string::npos){  //если задали путь,не содержащий ru-syntax-master.Сообщить и выйти.
		cout << "	Неверный путь до директории. Работа программы завершена.";
		system("pause");
		exit(0);
	}

	writable = new char[name.size() + 1];
	copy(name.begin(), name.end(), writable);
	writable[name.size()] = '\0';
	temp = new char[strlen(writable) + 10];
	while (i <= name.size()){ // если встретили просто одинарный слеш.То добавляем к нему еще один
		if (name[i] == '\\'){
			if (name[i + 1] != '\\'){
				strcpy(temp, substr(writable, i + v));
				writable[i + v] = '\0';
				strcat(writable, "\\");
				strcat(writable, temp);
				v = v + 1;
			}
	...
	if (answer == "N"){ // выбран файл по умолчанию.
		...
		config_write(writable);// заполняем config.ini с учетом введенного пути
	}
	else{
	...
	string line16 = "python install.py " + string(writable);//инструкция,какие программы нужно установить,есть возможность поставить необходимую библиотеку 
	...
	if (text == "auto"){ //выбран режим с ручным вводом текстов
		string line6 = "python clean.py " + string(writable) + " auto " + to_string(1);//запускаем скрипт, который удаляет временные файлы
		const char *line_6 = line6.c_str();
		system(line_6);
		autoanalyzer(writable, name);
	}

	if (text == "test"){ // если выбран режим тестирования
		test_analyzer(writable, name);
	}

	if (text == "add"){ /*режим добавления для тестов (полуавтоматический) и данных в словари */
		addmode(writable, name);
	}
	...
	if (text == "options"){
		...
		const char *line_19 = line19.c_str();
		system(line_19); /* выполняем clean.py */
		string option = string(writable) + +"\\" + "bin" + "\\Malt\\" + "\\options.ini";
